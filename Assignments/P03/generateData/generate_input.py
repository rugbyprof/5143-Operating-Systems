...
"""This file generates a data file for the simulation.  The file is in the format:
    Arrival time,  Process ID, Number of CPU bursts (N), Bursts (cpu , io, ...)
    The file is generated with random values for the following parameters:
...
def generate_file(**kwargs):
    ...
    maxCpuBT = int(
        kwargs.get("maxCpuBT", random.randint(int(minCpuBT) + 3, int(minCpuBT) + 8))
    )
