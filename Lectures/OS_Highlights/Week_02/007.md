...
paginate: true
header: "🐴 MSU OpSys"
footer: "Reader–Writer Problem — Griffin | GPT-5"
style: |
...
> “You’ve solved chaos by banning everyone from the library.”
...
def writer(id):
    ...
    while True:
        rw_mutex.acquire()
        data += 1
        print(f"✏️ Writer {id} updates data to {data}")
        rw_mutex.release()
        time.sleep(random.uniform(0.5, 1.0))
```
...
> “Endless readers = hangry writers.”

---

## ✍️ Attempt #4 – Writer-Priority Solution
